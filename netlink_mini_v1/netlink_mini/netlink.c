...
try_again:
	...
	if (left < 0) {
		if (errno != EINTR && errno != EAGAIN)
			ulog("netlink: recvfrom failed: %s", strerror(errno));
		return;
	}
	...
	if (left > 0) {
		ulog("netlink: %d extra bytes in the end of netlink message", left);
	}
...
struct netlink_data * netlink_init(struct netlink_config *cfg)
{
	...
	if (netlink->sock < 0) {
 		ulog("netlink: Failed to open netlink socket: %s", strerror(errno));
	...
	{
		ulog("netlink: Failed to bind netlink socket: %s", strerror(errno));
		netlink_deinit(netlink);
		return NULL;
	}
			   ...
			   int linkmode, int operstate)
{
	...
	ulog("netlink: Operstate: linkmode=%d, operstate=%d", linkmode, operstate);

	ret = send(netlink->sock, &req, req.hdr.nlmsg_len, 0);
	if (ret < 0) {
		ulog("netlink: Sending operstate IFLA failed: %s (assume operstate is not supported)", strerror(errno));
	}
